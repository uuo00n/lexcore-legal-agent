...
async def test_legal_search_returns_rerank_scores(monkeypatch):
    from mcp_server.tools import search

    monkeypatch.setattr(
        "services.search.get_retriever",
    ...
    payload = json.loads(await search.legal_search("保证责任 提前收回贷款"))
...
async def test_legal_search_marks_low_quality_when_top_score_below_threshold(monkeypatch):
    from mcp_server.tools import search
    ...
    payload = json.loads(await search.legal_search("新增执行案件 重大不利变化"))
